...
def main():
    ...
    rounds_p1 = np.arange(1, SHIFT_INTERVAL + 1)
    rounds_p2 = np.arange(1, TOTAL_ROUNDS + 1)

    # ── Panel 1: Field adaptation & passive alignment ─────────
    ax = axes[0]
    ax.plot(rounds_p1, f_mean, color="tab:green", label="Field")
    ax.fill_between(rounds_p1, f_mean - f_std, f_mean + f_std,
                    color="tab:green", alpha=0.2)
    ax.plot(rounds_p1, p_mean, color="tab:orange", label="Passive agents")
    ax.fill_between(rounds_p1, p_mean - p_std, p_mean + p_std,
                    color="tab:orange", alpha=0.2)
    ax.axhline(0, color="black", linewidth=0.5, linestyle="--")
    ax.set_xlabel("Round", fontsize=9)
    ax.set_ylabel("Cosine similarity to new concept", fontsize=9)
    ax.set_title("")
    ...
    # ── Panel 2: Repeated concept shifts ─────────────────────
    ax = axes[1]
    ax.plot(rounds_p2, hd_mean, color="tab:blue",
